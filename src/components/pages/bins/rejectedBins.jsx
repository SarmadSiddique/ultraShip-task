    ...
    >
      <main className="lg:container p-4 mx-auto" style={{ minHeight: '90vh' }}>
        <div className="flex items-center mb-3 gap-3">
          <h5 className="plusJakara_semibold text_dark">Rejected Bins</h5>
        </div>
        {loading ? (
      ...
      >
        <div className="flex mb-4 justiy-between items-center w-full">
          ...
          <div className="me-4">
            <span
              style={{
                border: adminBinData[currentPage]?.verify === true ? '1.5px solid #0EBE3C' : '1.5px solid #d90429',
                color: adminBinData[currentPage]?.verify === true ? '#0EBE3C' : '#d90429', fontSize: '12px'
              }}
              className="bg-transparent rounded-3 px-2 py-1 plusJakara_semibold">
              {adminBinData[currentPage]?.verify === true ? 'Approved' : 'Rejected'}
            </span>
          </div>
        </div>
        {loadingBin ? (
            ...
            <div className="flex gap-3 items-center">
              {adminBinData[currentPage]?.verify === true ? <>
                ...
                adminBinData[currentPage]?.rejected === true ? <>
                  ...
                  <button disabled={loading2} style={{ minWidth: '150px' }} onClick={() => handleUpdate(adminBinData[currentPage], 'approve')} className="px-4 bg_primary py-3 whitespace-nowrap rounded-4 text_white inter_regular">{loading2 && status === 'approve' ? <CircularProgress size={16} className="text_white" /> : 'Approve Again'}</button>
                </> : <>
                  <button disabled={loading2} onClick={() => handleUpdate(adminBinData[currentPage], 'approve')} className="px-4 py-3 rounded-4 bg_primary text_white inter_regular">{loading2 && status === 'approve' ? <CircularProgress size={16} className="text_white" /> : 'Approve'}</button>
                  <button disabled={loading2} onClick={() => handleUpdate(adminBinData[currentPage], 'reject')} style={{ backgroundColor: '#F3F3F3' }} className="px-4 py-3 rounded-4 text_dark inter_regular">{loading2 && status === 'reject' ? <CircularProgress size={16} className="text_white" /> : 'Reject'}</button>
                </>}
            </div>
          </div>}
