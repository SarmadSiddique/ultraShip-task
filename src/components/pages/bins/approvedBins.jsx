    ...
    try {
      ...
      if (res?.data) {
        setadminBinData(prevData => prevData.map((bin, index) =>
          bin._id === adminBinData[currentPage]?._id ? { ...bin, rejected: true, verify: false } : bin
        ));
        await getBinCount().then((res) => {
          if (res.status === 200) {
            dispatch(setBinCounts(res?.data))
    ...
    >
      <main className="lg:container p-4 mx-auto" style={{ minHeight: '90vh' }}>
        <div className="flex items-center mb-3 gap-3">
          <h5 className="plusJakara_semibold text_dark">Approved Bins</h5>
        </div>
        {loading ? (
      ...
      >
        <div className="flex mb-4 justiy-between items-center w-full">
          ...
          <div className="me-4">
            <span
              style={{
                border: adminBinData[currentPage]?.rejected === true ? '1.5px solid #d90429' : '1.5px solid #0EBE3C',
                color: adminBinData[currentPage]?.rejected === true ? '#d90429' : '#0EBE3C', fontSize: '12px'
              }}
              className="bg-transparent rounded-3 px-2 py-1 plusJakara_semibold">
              {adminBinData[currentPage]?.rejected === true ? 'Rejected' : 'Approved'}
            </span>
          </div>
        </div>
        {loadingBin ? (
          ...
          <div className="flex gap-3 mt-3 justify-between w-full">
            ...
            <div className="flex gap-3 items-center">
              {adminBinData[currentPage]?.verify === true ? <>
                ...
                <button disabled={loading2} onClick={() => handleUpdate(adminBinData[currentPage], 'reject')} style={{ backgroundColor: '#d90429', minWidth: "150px" }} className="px-4 py-3 rounded-4 whitespace-nowrap text_white inter_regular">{loading2 && status === 'reject' ? <CircularProgress size={16} className="text_white" /> : 'Reject Again'}</button>
              </> :
      ...
      >
        ...
        >
          <Input
            type="textarea"
            required
            className="form-control w-full text_secondary plusJakara_regular"
