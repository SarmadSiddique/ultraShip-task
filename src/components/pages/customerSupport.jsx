...
import { getAllSupports, updateSupport } from '../api/support';
...
const CustomerSupport = () => {
    ...
    const handleUpdate = async (row) => {
        ...
        try {
            const res = await updateSupport(row?._id)
            if (res.status === 200) {
            ...
            cell: (row) => {
                return (
                    row?.attended === true ?
                        ...
                        <button disabled={isProcessing} onClick={() => handleUpdate(row)} style={{ backgroundColor: '#2B7F75', padding: '6px 20px' }} className="text_white flex justify-center rounded-3 items-center">{isProcessing && statusId === row?._id ? (
